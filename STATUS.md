...
>
> Last updated: 2026-04-29 (WAVE-2026-04-29-53 truth-sync)
...
| S7 Smart-Home Hub | 🟡 G4 runway active | the hardware runway, `x86_64`-first bring-up plan, first Home Assistant bridge runway contract, first Pack B bring-up worksheet, operator rehearsal notes, a library-backed local-only `ferros-hub` crate, manifest-backed local bridge registration, `ferros-core` policy-backed allow/deny evaluation, a typed local runtime summary plus bounded `.tmp/hub` restart snapshot seam, restart-aware `summary | prove-bridge` outputs, additive optional `hubRestart` context on the existing read-only `/runway-summary(.json)` seam, display-only localhost shell and inspector rendering on the existing route, same-origin acceptance-harness proof, bounded local restart snapshot schema with H1 validator coverage, and `cargo xtask hub-runway` snapshot write/reload proof are now in repo; pairing semantics stay provisional while real Home Assistant proof, physical-device evidence, remote transport, and launch-grade durable hub runtime evidence remain open | G4 |
...
| S7 runway is no longer blocked on G3, and the local `ferros-hub` library seam, manifest/policy-backed simulated bridge proof path, bounded `.tmp/hub` restart snapshot, restart-aware summary/CLI output, additive read-only `/runway-summary(.json)` `hubRestart` observation, localhost shell plus same-origin acceptance proof, schema/H1 coverage, xtask helper, and emitted local artifacts are now in repo, but real Home Assistant proof, durable runtime behavior on target hardware, and physical-device evidence are still absent | S7 | S7 |
