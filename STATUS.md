...
>
> Last updated: 2026-04-28 (WAVE-2026-04-28-37 truth-sync)
...
| S7 Smart-Home Hub | 🟡 G4 runway active | the hardware runway, `x86_64`-first bring-up plan, first Home Assistant bridge runway contract, first Pack B bring-up worksheet, operator rehearsal notes, the first local-only `ferros-hub` binary scaffold, the first crate-local bridge seam, and a focused local allow/deny/error proof loop with emitted `.tmp/hub/simulated-local-bridge-artifact.json` are now in repo; pairing semantics stay provisional while real Home Assistant proof, physical-device evidence, remote transport, and durable hub runtime evidence remain open | G4 |
...
| S7 runway is no longer blocked on G3, and the local `ferros-hub` scaffold, crate-local bridge seam, simulated proof loop, and emitted local artifact are now in repo, but real Home Assistant proof, durable runtime behavior on target hardware, and physical-device evidence are still absent | S7 | S7 |
