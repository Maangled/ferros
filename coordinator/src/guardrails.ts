...
export class GuardrailChecker {
   ...
   */
  static async checkAll(packet: Packet, targetAgent: 'core' | 'subcore'): Promise<GuardrailResult> {
    const checks: Record<string, GuardrailCheckResult> = {};
   ...
   */
  static async check1_PacketValid(packet: Packet, targetAgent: 'core' | 'subcore'): Promise<GuardrailCheckResult> {
    try {
      // Structural validation
      const validation = PacketValidator.validatePacket(packet);
      if (!validation.valid) {
        return {
      ...
      // Target stream match
      const streamMatch = PacketValidator.checkTargetStreamMatch(packet, targetAgent);
      ...
      const continuity = PacketValidator.checkRunIdContinuity(packet);
      if (!continuity.valid) {
        return {
          name: 'check1_PacketValid',
          passed: false,
   ...
   */
  static async check2_RecursionDepth(packet: Packet): Promise<GuardrailCheckResult> {
   ...
   */
  static async check3_ParentRunId(packet: Packet): Promise<GuardrailCheckResult> {
   ...
   */
  static async check4_TTLValid(packet: Packet): Promise<GuardrailCheckResult> {
   ...
   */
  static async check5_SelfHandoffPrevention(
    packet: Packet,
