...
exports.EventTracer = void 0;
exports.getEventTracer = getEventTracer;
const LIFECYCLE_SECTION_PATTERNS = [
    {
        kind: 'work_order',
        label: 'Work Order',
...
class EventTracer {
    constructor() {
        this.events = [];
        this.toolCallIdMap = new Map(); // toolCallId -> parent_run_id
    }
    /**
     * Setup event listeners on SDK session
     * Returns mapping of events captured
     */
    setupEventListeners(session, packet) {
        let lastToolCallId;
     ...
     */
    normalizeResponse(response, packet, toolCallId) {
        const targetStream = packet.route_token.target_stream;
        const classification = targetStream === 'core' ? 'execution-return-core' : 'execution-return-subcore';
     ...
     */
    extractLifecycleOutcome(response, packet) {
        const matches = [];
        ...
        const outcome = {
            ...
            target_agent_id: match.kind === 'escalation' ? contract?.escalation_target_agent_id : undefined,
            stop_reason: match.kind === 'stopped' ? match.summary : undefined,
        };
        return { outcome };
    }
    extractSectionSummary(section) {
        const lines = section
            .split('\n')
            .map((line) => line.trim())
            .filter(Boolean);
        ...
        const bulletLine = lines.find((line) => line.startsWith('-') || line.startsWith('•'));
