...
exports.PacketValidator = void 0;
...
class PacketValidator {
     ...
     */
    static validateLifecycleContract(packet) {
        const errors = [];
        ...
        const invalidStates = contract.stop.allowed_terminal_states.filter((state) => !VALID_TERMINAL_STATES.includes(state));
        if (invalidStates.length > 0) {
            errors.push(`Invalid lifecycle_contract.stop.allowed_terminal_states: ${invalidStates.join(', ')}`);
        }
        if (contract.stop.allowed_terminal_states.includes('stopped') &&
            contract.stop.stopped_reason_required !== true) {
            errors.push("lifecycle_contract.stop.stopped_reason_required must be true when 'stopped' is allowed");
        }
        ...
        if (packet.route_token.target_stream &&
            ...
            errors.push(`lifecycle_contract.target_agent_id '${contract.target_agent_id}' does not match route_token.target_stream '${packet.route_token.target_stream}'`);
        }
     ...
     */
    static validateExecutionOutcome(packet, executionReturn) {
        const errors = [];
        const lifecycleValidation = this.validateLifecycleContract(packet);
        if (!lifecycleValidation.valid) {
            errors.push(...lifecycleValidation.errors);
        ...
        if (contract &&
            ...
            errors.push(`Lifecycle outcome '${outcome.kind}' is not allowed by the packet stop contract`);
        }
        if (outcome.kind === 'work_order' &&
            contract?.work_order_id &&
            outcome.work_order_id &&
            outcome.work_order_id !== contract.work_order_id) {
            errors.push(`Lifecycle work_order_id '${outcome.work_order_id}' does not match contract '${contract.work_order_id}'`);
        }
        if (outcome.kind === 'escalation') {
            ...
            if (contract?.escalation_id &&
                outcome.escalation_id &&
                outcome.escalation_id !== contract.escalation_id) {
                errors.push(`Lifecycle escalation_id '${outcome.escalation_id}' does not match contract '${contract.escalation_id}'`);
            }
        }
     ...
     */
    static checkRunIdContinuity(packet, _priorRunId) {
        const errors = [];
     ...
     */
    static checkTargetStreamMatch(packet, expectedStream) {
        const errors = [];
        if (packet.route_token.target_stream !== expectedStream) {
            errors.push(`target_stream mismatch: packet targets '${packet.route_token.target_stream}' but handoff is to '${expectedStream}'`);
        }
        return { valid: errors.length === 0, errors };
    }
}
exports.PacketValidator = PacketValidator;
