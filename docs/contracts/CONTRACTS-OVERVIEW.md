...
| Local Onramp Proposal | S6 | `schemas/onramp-proposal.schema.json` | S7 (local artifact emission), S1 support (H1 validator), S8 (contracts truth) | 🟡 Local-only quarantined proposed-material schema aligned to `ferros-data`'s `LocalOnrampProposal`; emitted by S7 as `.tmp/hub/local-onramp-proposal.json`, H1-covered, and intentionally not an accept/reject flow, grant, canonical mutation, or gate-closing contract |
