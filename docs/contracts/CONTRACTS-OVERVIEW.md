...
| Local Onramp Proposal | S6 | `schemas/onramp-proposal.schema.json` | S7 (local artifact emission), S4 (runway summary projection), S5 (shell and harness observation), S1 support (H1 validator), S8 (contracts truth) | 🟡 Local-only quarantined proposed-material schema aligned to `ferros-data`'s `LocalOnrampProposal`; emitted by S7 as `.tmp/hub/local-onramp-proposal.json`, projected by S4 on the existing runway-summary seam, H1-covered, and intentionally not an accept/reject flow, grant, canonical mutation, or gate-closing contract |
| Local Onramp Decision Receipt | S6 | `schemas/onramp-decision-rehearsal.schema.json` | S7 (local artifact emission), S4 (runway summary projection), S5 (shell and harness observation), S1 support (H1 validator), S8 (contracts truth) | 🟡 Local-only recorded decision-rehearsal schema aligned to `ferros-data`'s `LocalOnrampDecisionReceipt`; emitted by S7 as `.tmp/hub/local-onramp-decision-receipt.json`, projected by S4 on the existing runway-summary seam, H1-covered, and intentionally not an executed consent event, accept/reject transport, grant, canonical mutation, or gate-closing contract |
| AgentManifest | S3 | `schemas/agent-manifest.v0.json` | S4, S5, S7 | ⬜ Not yet created |

The local-only runway schema rows above are shared with harness/docs surfaces only. `Hub Local State Snapshot` remains the S7-owned restart-context contract, while `Local Onramp Proposal` and `Local Onramp Decision Receipt` remain S6-owned local rehearsal models that S7 emits as bounded local artifacts and S4/S5 observe on the current localhost path. None of these rows imply Home Assistant integration, hardware evidence, remote transport, or G4 closure.
...
| Localhost runway summary surface (`GET /runway-summary(.json)`) | S4 | S5 (shell, harness), S7 (runway planning) | 🟡 Local-only read-only route served from `crates/ferros-node/src/lib.rs`; it now additively carries optional `hubRestart`, `hubOnrampProposal`, and `hubOnrampDecisionReceipt` context sourced from the hub summary seam so the current localhost path can observe bounded restart state, pending-consent proposed material, and the recorded decision rehearsal receipt without publishing a durable hub restart API, accept/reject transport, remote transport, or gate-closing evidence |
| Localhost JSON/RPC contract (agent center) | S3 | S5 Phase B (web shell) | 🟡 Defined in `crates/ferros-agents/src/rpc.rs` and hosted locally via `crates/ferros-node/src/lib.rs`; the current read methods remain `agent.list`, `agent.describe`, `agent.snapshot`, `grant.list`, and `denyLog.list`, and the first local-only lifecycle/write methods `agent.run` and `agent.stop` now route through `LocalAgentApi` on the same localhost host and state path; the shell now uses the read seam plus that narrow localhost-only lifecycle slice, while broader browser control, remote transport, grant writes, and richer remote observation/control remain open |
