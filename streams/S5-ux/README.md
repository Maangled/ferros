...
**Stream:** S5  
**Status:** 🟨 Phase A active on the real landing page; Phase B localhost shell slice landed with narrow lifecycle/profile controls, display-only onramp observation, operator-facing blocked/recovery posture, and first touch-screen shell posture
**Gate:** Contributes to launch-readiness; no blocking gate owned solely by S5

> Current checkpoint: the first localhost browser profile surface is landed for `init`, `show`, `export`, and `import` through the local `/profile` adapter with focused route-test coverage, and the existing runway route now renders pending-consent proposed material plus the recorded local decision rehearsal receipt from `/runway-summary.json` as display-only, local-only, non-evidentiary observation with same-origin H9 proof. The first shared-module batch is now also live on the current shell: `ProposedMaterialCard`, `ReceiptStrip`, `EvidenceBadge`, `SourceLineageCard`, `OperatorStepCard`, and `ToolLaneCard` are rendering the runway and route-scope surfaces on the served localhost shell with route-test and H9 marker coverage. The first shell-frame extraction slice is now landed on that same shell: protected-chrome markers are live, route metadata is single-homed, and the top edge, bottom status rails, plus side-panel headers now render through shared shell renderers. `LifecycleControlCard` and `LifecycleOutcomeCard` are also landed on the current shell, and the runway route now also carries `ConsentBoundaryCard` plus `RecoveryStateCard` so blocked canonical state, route-local artifacts, and the next honest recovery move stay explicit without faking accept/reject writes. The first touch posture follow-up is now landed there too: a protected `TouchAnchorStrip`, coarse-pointer tap sizing, and sticky top/bottom context now make the current Linux-host shell more honest for a USB-C operator screen without claiming subcore driver integrity. The same shell now also exposes `Home-Hub`, `Forge`, and `Arena` as bounded read or preview routes on top of the state it already owns, so topology, preview-only artifact posture, and non-evidentiary runtime posture are visible without widening writes. Focused `ferros-node` tests passed for the route expansion, and grouped H1-H4 human-session packets are now ready to issue for the non-hardware waves while HTB-013 remains the hardware-backed touch pilot. Remaining S5 follow-up is now grouped as issued human-test packets for the current runway and preview slices, a real host-touchscreen pilot on this machine, and later onramp accept/reject wiring without reopening the frozen S2 contract or widening browser privileges. The broader frontend-first program is now staged in `docs/orchestration/INTERCONNECTED-OPERATOR-UX-PLAN.md`, with PDF-derived motif input coming from the external discovery packet at `/home/homelab001/apps/FERROS-PDF-DISCOVERY-NOTE.md`.
...
The current Phase B slice is still read-first for observation, with one narrow localhost-only lifecycle write bar, one narrow localhost-only profile surface, and one display-only onramp observation path on the existing runway route. That profile slice is landed for `init`, `show`, `export`, and `import` through the local `/profile` adapter only, and it is already backed by focused `ferros-node` route tests and Rust validation. The shell now also carries the first shared-module batch over those existing surfaces: `ProposedMaterialCard`, `ReceiptStrip`, `EvidenceBadge`, `SourceLineageCard`, `OperatorStepCard`, and `ToolLaneCard` are live on the served localhost shell and guarded by focused route tests plus H9 marker checks. The first shell-frame extraction seam is now live there too: protected top-edge, bottom-edge, status-rail, panel-header, and collapse markers are emitted on the served shell, route metadata is single-homed, and the top/bottom/status chrome plus the side-panel headers now render behind shared shell renderers. The runway route now also exposes `ConsentBoundaryCard` and `RecoveryStateCard` so the blocked consent boundary and operator recovery posture stay visible on the same read-only path, and the protected top chrome now carries `TouchAnchorStrip` plus coarse-pointer posture rules so the current narrow layout is usable as a dedicated touch screen rather than only as a fallback breakpoint. The remaining S5 follow-up keeps `show` off JSON-RPC and leaves profile `grant` / `revoke`, onramp accept/reject wiring, remote transport, and broader browser privileges out of scope.

The shell now also stages three additional bounded routes on that same state: `Home-Hub` reads topology and lineage posture from the loaded runway summary, `Forge` stages preview-only bundle and artifact posture from the local `/profile` surface plus route-local artifacts, and `Arena` stages preview-only runtime posture from the selected agent plus runway recovery state. They are all intentionally honest surfaces: no hidden writes, no canonical publish, no automatic promotion into Profile, and no Home Assistant or hardware truth claims.

The shell now stages selected-agent lifecycle intent copy through shared tool-lane surfaces and can submit selected-agent `agent.run` / `agent.stop` only through the grant-aware local lifecycle bar; the current UI is still effectively one writer lane.

## Current shell-frame, runway recovery, and touch posture slice

The current shell-side follow-up above the landed localhost `agent.run` / `agent.stop` backend slice now includes the first extracted shell frame, shared lifecycle cards, a read-only runway boundary/recovery layer, and the first dedicated touch posture follow-up:

- Keep browser-issued writes scoped to the currently selected agent on the current localhost shell only; do not invent a second writer lane, remote transport, or broader browser control.
- Keep route metadata single-homed and the top edge, bottom status rails, side-panel headers, plus stable protected-chrome markers behind shared shell renderers so extraction progress stays explicit and testable.
- Use `LifecycleControlCard` and `LifecycleOutcomeCard` on the current shell while preserving the existing lifecycle gate ids, arm checkbox, and read-after-write refresh through `agent.snapshot`.
- Use `ConsentBoundaryCard` on the runway route to keep accept/reject closed, canonical state blocked, and the route-local artifact path explicit on the existing read-only `/runway-summary.json` surface.
- Use `RecoveryStateCard` on the runway route to stage deny-driven recovery posture, restart visibility, checkpoint state, and the next honest operator move without implying accepted onramp state.
- Use `TouchAnchorStrip` plus coarse-pointer sizing so routes, focus, inspector, tools, and audit surfaces stay persistently reachable on a dedicated touchscreen without introducing hover-only navigation.
- Treat the current Linux-host touchscreen path as host-side observation only: record the cable path and driver stack during human testing, but do not treat host display/input drivers as FERROS-managed evidence until subcore driver work exists.
- Keep grant/revoke actions, onramp accept/reject controls, consent resolution for non-lifecycle operations, and broader S4 restart/reload semantics out of scope until later code-backed follow-up exists.
...
1. Issue the first operator-ready review pass on the current runway slice through HTB-004 so a human can inspect the proposed-material route, the blocked consent boundary, and the recovery posture that just landed.
2. Run the first host-touchscreen pilot on this Linux machine: prefer the single-cable motherboard USB-C path when available, otherwise record the explicit fallback cable path, and capture the host display/input driver metadata in the operator findings.
3. Issue the grouped H1-H4 operator packets for the non-hardware waves, then prove the first operator-session loops end to end on the localhost shell without implying that onramp accept/reject is already wired or that the shell has more than one effective writer lane.
4. Land the first touch-critical consent flow only after the audit-log seam can record an explicit accept event, with a visible receipt and no silent path to canonical state.
5. Finish the non-UX hygiene already in flight and keep new shared modules plus route language aligned with `docs/orchestration/INTERCONNECTED-OPERATOR-UX-PLAN.md`, the new touchscreen-driver ADR, and the discovery-note quarantine rules.
