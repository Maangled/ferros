...
- [x] Close out the landed localhost `/profile` adapter path with focused `ferros-node` route-test coverage and Rust validation, keeping scope to `init`, `show`, `export`, and `import` only without reopening S2 or widening browser privileges
...
- [x] Land display-only proposal plus decision-rehearsal observation on the existing runway route and prove it in the same-origin H9 harness without accept/reject controls, canonical mutation, or route widening
