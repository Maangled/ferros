...
- [x] Land the first localhost shell profile slice for `init`, `show`, `export`, and `import` only through the local `/profile` adapter, with `show` staying off JSON-RPC, `/profile` checked separately from `/rpc`, profile `grant` / `revoke` controls absent, and the frozen S2 boundary unchanged
- [ ] Close out the landed localhost `/profile` adapter path with focused `ferros-node` route-test coverage and Rust validation when the environment can execute it, keeping scope to `init`, `show`, `export`, and `import` only without reopening S2 or widening browser privileges
...
- [x] Land display-only proposed-material observation on the existing runway route and prove it in the same-origin H9 harness without accept/reject controls, canonical mutation, or route widening
