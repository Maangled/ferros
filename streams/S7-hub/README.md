...
3. **Local proof loop exists** — targeted local tests, a bounded `LocalHubStateSnapshot` seam under `.tmp/hub/`, bounded `.tmp/hub/local-onramp-proposal.json` and `.tmp/hub/local-onramp-decision-receipt.json` artifacts, a restart-aware `LocalHubRuntimeSummary`, thin `summary | prove-bridge | deny-demo` CLI proofs, additive optional `hubRestart`, `hubOnrampProposal`, and `hubOnrampDecisionReceipt` children on the existing read-only `/runway-summary(.json)` seam, display-only localhost shell and inspector rendering plus same-origin acceptance-harness proof on that same route, bounded local artifact/report/restart-snapshot/proposal-and-decision schemas in the H1 validator, and `cargo xtask hub-runway` proving snapshot write/reload while validating and reporting the proposal and decision artifacts. This remains local-only runway proof and does not imply D1 or G4 closure.
...
| Path | Role | Status |
|------|------|--------|
| `crates/ferros-hub/` | Hub binary crate | 🟨 Library-backed local-only runway, restart snapshot seam, local onramp proposal plus decision artifacts, CLI proofs, and runtime summary/helper surfaces landed |
