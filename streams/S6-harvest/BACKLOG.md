...
- [x] Publish the typed local onramp decision receipt boundary in `ferros-data` with bounded local-only validation/write helpers for downstream hub consumers
- [x] Hand off the local onramp decision receipt boundary to the current S7/S4/S5 rehearsal packet without reopening ADR-020 or inventing a second local decision model
